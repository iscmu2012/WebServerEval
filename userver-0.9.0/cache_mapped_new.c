...
cache_cleanup(void)
{
  ...
  if (options.use_socket_aio) {
    ...
    PRINT_TIME(NOFD, &tnow, &tprev,
      "cache_cleanup: aio_sock_mem_dereg rc = %d", rc);
...
#ifdef HAVE_AIO_LAYER
          if (options.use_socket_aio) {
            aio_sock_mem_dereg(c->encryptbuf, size + 8,
              c->encrypt_buf_handle);
          }
#endif /* HAVE_AIO_LAYER */
...
printBucket(int index, int which)
{
  ...
  printf("%7d %40s %3d %6d %10d %6.3lf%% %10d %6.3lf%% %10d %6d %6d %6d\n",
    ...
    tmp->reply_count,
    (double) tmp->reply_count / (double) tmp->use_count * 100.0, tmp->buf_len,
    next, prev, tmp->hash_to);
}
...
#define FILLBUFSZ 2048
  ...
  if (found == 0) {
    ...
    TRACE(EVT_OPEN_FILE, trace_fd = 0; fd = open(uri, O_RDONLY); rc = fd;);
    ...
    TRACE(EVT_FSTAT, trace_fd = fd; rc = fstat(fd, &fileStat););
    ...
    if (dbl_filesize > cache.maxFileSize || dbl_filesize > cache.maxBytes) {
      PRINT_TIME(NOFD, &tnow, &tprev, "File too big to cache!");
      PRINT_TIME(NOFD, &tnow, &tprev,
        "filesize = %d maxFileSize = %.0f maxBytes = %.0f\n",
    ...
    if (!options.use_sendfile || options.use_madvise
      || options.victim_skip == OPT_SKIP_DO_MADVISE) {
      if (fileStat.st_size > 0) {
        TRACE(EVT_MMAP,
          trace_fd = fd;
          cur->buf = (char *) mmap(0, fileStat.st_size, PROT_READ,
            MAP_SHARED, fd, 0); rc = (cur->buf != MAP_FAILED););
        assert(cur->buf != MAP_FAILED);
        PRINT_TIME(NOFD, &tnow, &tprev, "cache_add: mmap returns %p",
          cur->buf);
      }
    }
...
#ifdef HAVE_AIO_LAYER
        PRINT_TIME(NOFD, &tnow, &tprev,
          "cache_add: aio_sock_mem_reg start %p size = %d ", cur->buf,
          fileStat.st_size);
    ...
    if ((options.encrypt_data) &&
      ...
      if ((options.use_socket_aio) || (!options.use_sendfile)) {
        ...
        if (options.use_socket_aio) {
          ...
          if (rc < 0) {
            printf
              ("cache_add: aio_sock_mem_reg failed (encryption buffer)\n");
            exit(1);
          }
        }
#endif /* HAVE_AIO_LAYER */
    ...
    if (!options.use_sendfile && cache.lockPages != 0) {
      TRACE(EVT_MLOCK, trace_fd = fd; rc = mlock(cur->buf, cur->buf_len););
      assert(rc != -1);
    }
...
#define USE_LRU
  ...
  if (victim == NULL_INDEX) {
    ...
    printf
      ("cache_victim: Increase the number of cache table entries or the total cache size and retry\n");
  ...
  if (!options.use_sendfile) {
    if (cache.lockPages != 0) {
      /* Evict this page */
      TRACE(EVT_MUNLOCK, trace_fd = 0; rc = munlock(cur->buf, cur->buf_len););
    }

    TRACE(EVT_MUNMAP, trace_fd = 0; rc = munmap(cur->buf, cur->buf_len););

    if (options.use_socket_aio) {
#ifdef HAVE_AIO_LAYER
      PRINT_TIME(NOFD, &tnow, &tprev,
        "cacheVictim: aio_sock_mem_dereg start %p size = %d " "handle = %p",
...
#ifdef HAVE_AIO_LAYER
      if (options.use_socket_aio) {
        aio_sock_mem_dereg(cur->encryptbuf, cur->buf_len + 8,
          cur->encrypt_buf_handle);
      }
#endif /* HAVE_AIO_LAYER */
      ...
      if (cur->reference == 0) {
        ...
        if (!options.sendfile) {
          if (cache.lockPages != 0) {
            /* Evict this page */
            TRACE(EVT_MUNLOCK,
              trace_fd = 0; rc = munlock(cur->buf, cur->buf_len););
          }

          TRACE(EVT_MUNMAP,
            trace_fd = 0; rc = munmap(cur->buf, cur->buf_len););
...
cache_dec_ref_count(struct cacheinfo *c)
{
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "cache_dec_ref_count: c = %p ref_count = %d", c, c->ref_count);
...
cache_inc_ref_count(struct cacheinfo *c)
{
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "cache_inc_ref_count: c = %p ref_count = %d", c, c->ref_count);
}
...
cache_it(const char *fname, const char *base)
{
  ...
  if (!(stride >= options.cache_warm_size)) {
    printf
      ("cache_it: stride must be greater than or equal to the --cache-warm-size\n");
    exit(1);
  }
#endif
  ...
  if (S_ISDIR(statinfo.st_mode)) {
    ...
    for (i = 0; i < options.cache_warm_loops; i++) {
      ...
      for (x = 0; x < info->buf_len; x += stride) {
        ...
        ldbg
          ("cache_it: read_size = %10zu cur byte = %10zu bytes_left = %10zu\n",
 ...
 * need to be stat'd again.
 ------------------------------------------------------------------------*/
int
check_if_modified_since(cacheinfo * cur, time_t since)
{
  ...
  if (cur->last_stat <= since) {
    ...
    TRACE(EVT_FSTAT, trace_fd = cur->fd; rc = fstat(cur->fd, &fileStat););
    if (rc < 0) {
      printf
        ("cache_mapped_new.c: check_if_modified_since: unable to stat fd =%d\n",
        cur->fd);
      exit(1);
    }
    num_extra_fstat++;
