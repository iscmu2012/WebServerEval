...
info_init(void)
{
  ...
  printf("info_init: sizeof(struct req)  = %d mod cache_line_size = %d "
    ...
    (int) sizeof(struct req) % cache_line_size, (int) sizeof(struct req) % 4);
  printf("info_init: sizeof(struct rep)  = %d mod cache_line_size = %d "
    "mod 4 = %d\n",
    (int) sizeof(struct rep),
    (int) sizeof(struct rep) % cache_line_size, (int) sizeof(struct rep) % 4);
  ...
  for (i = 0; i < num_info_structs; i++) {
    ...
    ip->req.tmp = NULL;
    ip->req.tmp_len = 0;
    ip->req.tmp_allocated = 0;
    ip->req.uri = NULL;
    ip->req.uri_len = 0;
    ip->req.uri_allocated = 0;
    ip->req.cookie = NULL;
    ip->req.cookie_len = 0;
    ip->req.cookie_allocated = 0;
#endif /* UMEMREDUCTION */
    ...
    ip->req.range_start = 0;
    ip->req.range_end = 0;
    ip->req.range_request = 0;
    ...
    if (info[i].sd > 0) {
      ...
      if ((info[i].type == INFO_LISTENER) ||
        ...
        ((info[i].type == INFO_FASTCGI) &&
          ((info[i].fsm_state != FSM_WRITING_REPLY))
          && (info[i].fsm_state != FSM_INITIAL_STATE))
...
#ifdef TRACK_STUCK_CONNS
        printf
          ("%6d type = %d sd = %6d sd_state = 0x%4x fsm_state = %2d history = 0x%08x prev history = 0x%08x "
        ...
        printf("%6d type = %d sd = %6d sd_state = 0x%4x fsm_state = %2d "
...
#endif /* TRACK_STUCK_CONNS */
          i, info[i].type, info[i].sd, info[i].sd_state, info[i].fsm_state,
#ifdef TRACK_STUCK_CONNS
          info[i].fsm_state_history,
          info[i].prev_fsm_state_history,
          info[i].read_count,
          info[i].read_nread, info[i].read_errno, info[i].session_len,
#endif /* TRACK_STUCK_CONNS */
          ...
          info[i].conn_num, info[i].rep.cache, info[i].req.close ? 0 : 1);
...
info_close_rep_fd(struct info *ip)
{
  ...
  PRINT_TIME(ip->sd, &tnow, &tprev,
    "info_close_rep_fd: ip->type = %d fd = %d", ip->type, ip->rep.fd);
  PRINT_TIME(ip->sd, &tnow, &tprev,
    "info_close_rep_fd: ip->req.type = %d ip->rep.use_cache = %d",
    ...
    case INFO_CLIENT:
      switch (ip->req.type) {
        ...
        case REQ_FASTCGI:
          ...
          PRINT_TIME(ip->sd, &tnow, &tprev,
            "info_close_rep_fd: service next request with peer_sd = %d",
            peer_sd);
...
info_reset_rep_buf(struct info *ip)
{
  ...
  if (ip->rep.dyn_buf != NULL) {
    ...
    PRINT_TIME(ip->sd, &tnow, &tprev, "app_unblock_next returns app = %p",
      app);
    ...
    if (app != NULL) {
      ...
      switch (app->req_type) {
        case REQ_FASTCGI:
          ...
          if (sd < 0) {
            PRINT_TIME(ip->sd, &tnow, &tprev,
              " no new clients requests to work on doing nothing");
            // fcgi_go_idle(ip);
          ...
          assert(sd >= 0);
          PRINT_TIME(ip->sd, &tnow, &tprev,
            " processing fcgi request on sd = %d", sd);
          fcgi_process_request(sd);
#endif /* FIXHUP */
          break;
        default:
          ...
          if (first_ip != 0) {
            if (first_ip != fcgi_ip) {
              PRINT_TIME(NOFD, &tnow, &tprev,
                "trying to service next request on %d", fcgi_ip->sd);
...
get_peer_addr_str(int sd, char *str, int maxlen)
{
  ...
  if (rc < 0) {
    ...
    snprintf(str, maxlen, "couldn't get peer for sd = %d errno = %d", sd,
      errno);
  } else {
    snprintf(str, maxlen, "%s:%d", inet_ntoa(name.sin_addr),
      ntohs(name.sin_port));
  }
}
