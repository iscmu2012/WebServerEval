...
do_write(int sd, struct rep *rep)
{
  ...
  if (options.use_aio_write) {
    ...
    PRINT_TIME(sd, &tnow, &tprev, "do_write: aio_sock_write buf = %p "
      "len = %d handle = %p", rep->cur, (rep->end - rep->cur),
      rep->write_reg_handle);
    TRACE(EVT_AIO_SOCK_WRITE, trace_fd = sd;
      rc =
      aio_sock_write(sd, rep->cur, (rep->end - rep->cur),
        rep->write_reg_handle););
    save_errno = errno;
    ...
    if (ip->ssl) {
      ...
      TRACE(EVT_WRITE_SOCK,
        trace_fd = sd; rc = write(sd, rep->cur, rep->end - rep->cur););
      save_errno = errno;
...
post_sock_write(int sd, int nwritten, int err)
{
  ...
  if (nwritten < 0) {
    ...
    PRINT_TIME(sd, &tnow, &tprev,
      "post_sock_write: write failed -  returning -1");
