...
static int listener_create(const char *path, int init_mode,
  int is_ssl_listener);
...
listener_send_async_setup(int sd)
{
  ...
  if (!options.accepts_only) {
    if (!options.send_polls_for_accepts) {
      ...
      DEBG(MSG_SEND,
        "listener_send_async_setup: calling fcntl group num = %d\n",
        IO_GRP_NUM);
...
listener_async_setup(int sd)
{
  ...
  DEBG(MSG_ASYNC, "Done setting up socket %d " "for asynchronous use\n", sd);
...
server_init(int init_mode)
{
  ...
  if (options.ip_addr_count == 0) {
    ...
    for (i = 0; i < options.ip_addr_count; i++) {
      sd = 0;
      sd =
        listener_create(options.ip_addr[i], init_mode, LISTENER_IS_NOT_SSL);
      post_listener_create(sd, init_mode);
      printf("listening socket is %d\n", sd);
    }
  }
...
listener_create(const char *path, int init_mode, int is_ssl_listener)
{
  ...
  if (is_ssl_listener) {
    ...
    PRINT_TIME(ssl_listen_sd, &tnow, &tprev,
      "listener_create: got server context\n");
  }
#endif /* HAVE_SSL */
...
server_extras(int listen_sd)
{
  ...
  PRINT_TIME(listen_sd, &tnow, &tprev, "server_extras: listen_sd = %d",
    listen_sd);
        ...
        if (options.use_epoll || options.epoll_edge_triggered ||
          ...
          for (i = 0; i < numprocs; i++) {
            ...
            TRACE(EVT_EPOLL_CTL,
              trace_fd = listen_sd;
              rc =
              sys_epoll_ctl(epoll_fd_array[i], EPOLL_CTL_ADD, listen_sd,
                &ep_evt); save_errno = errno;);
#else
            TRACE(EVT_EPOLL_CTL,
              trace_fd = listen_sd;
              rc =
              epoll_ctl(epoll_fd_array[i], EPOLL_CTL_ADD, listen_sd, &ep_evt);
              save_errno = errno;);
#endif
            ...
            PRINT_TIME(listen_sd, &tnow, &tprev,
              "server_extras: set epoll_array listen_sd = %d", listen_sd);
...
listener_setopts(int sd)
{
  ...
  DEBG(MSG_SOCKBUF, "after setsockopt lowat = %d\n", size);
#endif /* !SUNOS && !__CYGWIN__ && !__linux__ */

  optlen = sizeof(size);
  if (getsockopt(sd, SOL_SOCKET, SO_SNDBUF, (void *) &size, &optlen) < 0) {
  ...
  optlen = sizeof(size);
  if (getsockopt(sd, SOL_SOCKET, SO_RCVBUF, (void *) &size, &optlen) < 0) {
    perror("listener_opts: getsockopt");
    //close(sd);
    exit(-1);
  } else {
...
#if !defined(SUNOS) && !defined(__CYGWIN__) && !defined(__linux__)
  optlen = sizeof(size);
  if (getsockopt(sd, SOL_SOCKET, SO_SNDLOWAT, (void *) &size, &optlen) < 0) {
    perror("listener_opts: getsockopt");
    //close(sd);
    exit(-1);
  } else {
  ...
  optlen = sizeof(size);
  if (getsockopt(sd, SOL_SOCKET, SO_RCVLOWAT, (void *) &size, &optlen) < 0) {
    perror("listener_opts: getsockopt");
    //close(sd);
    exit(-1);
  } else {
...
fcgi_create(app_t * app, appserver_t * appserver)
{
  ...
  appserver->sd = sd = socket_create(addr.u.sa.sa_family, SOCK_STREAM, 0);
  if (sd < 0) {
  ...
  PRINT_TIME(sd, &tnow, &tprev,
    "fcgi_create: calling fcgi_get_server_values");
...
#endif
  PRINT_TIME(sd, &tnow, &tprev,
    "fcgi_create: calling fcgi_set_server_values");
  if (fcgi_set_server_values(appserver) < 0) {
    printf("fcgi_set_server_values failed -- exiting\n");
    exit(1);
  }

  PRINT_TIME(sd, &tnow, &tprev, "fcgi_create: successful created sd = %d",
    sd);
