    ...
    case OPT_SIGIO_USE_PROCMASK:
      ...
      if (in_handler) {
        ...
        TRACE(EVT_SIGPROCMASK,
          trace_fd = 0; rc = sigprocmask(0, NULL, &set););
        if (rc < 0) {
        ...
        sigaddset(&set, SIGIO);
        TRACE(EVT_SIGPROCMASK,
          trace_fd = 0; rc = sigprocmask(SIG_BLOCK, &set, &old););
        if (rc < 0) {
    ...
    case OPT_SIGIO_USE_FCNTL:
      ...
      for (i = sock_listener_min; i <= sock_listener_max; i++) {
        if (sock_is_listener(i)) {
          ...
          TRACE(EVT_FCNTL,
            trace_fd = i; rc = fcntl(i, F_SETFL, flags | O_NONBLOCK););
  ...
  switch (options.sigio_cntl) {
    case OPT_SIGIO_USE_PROCMASK:
      ...
      if (in_handler) {
        ...
        print_mask("do_unblock: uc_sigmask was \n", &uc->uc_sigmask);
        TRACE(EVT_CHANGESET,
          trace_fd = 0; rc = sigdelset(&uc->uc_sigmask, SIGIO););
        print_mask("do_unblock: uc_sigmask is now\n", &uc->uc_sigmask);
#endif /* MACROBUG */
      } else {
        ...
        sigaddset(&set, SIGIO);
        TRACE(EVT_SIGPROCMASK,
          trace_fd = 0; rc = sigprocmask(SIG_UNBLOCK, &set, &old););
        if (rc < 0) {
          perror("sigprocmask set");
          exit(1);
        }
...
block_send_events(void)
{
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "block_send_events: returning notify = 0x%x", ecb->notify);
}
...
unblock_send_events(void)
{
  ...
  DEBG(MSG_INTR,
    "unblock_send_events: done setting send_events_blocked = 0\n");
