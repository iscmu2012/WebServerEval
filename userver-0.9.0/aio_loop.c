  ...
  while (1) {
    ...
    if (n_total == 0) {
      ...
      if (options.aio_dont_wait) {
        ...
        do {
          ...
          if (n == AIO_WAIT_ERROR && aio_wait_errno == EINTR) {
            PRINT_TIME(NOFD, &tnow, &tprev, "aio_loop: aio was interrupted");
      ...
      if (options.close_idle_conns) {
        ...
        printf
          ("preposted accepts may cause the socket/connection to appear idle\n");
        exit(1);
...
handle_completions(int queue, int n)
{
  ...
  TRACE(EVT_AIO_SOCK_GETEVENTS,
    trace_fd = queue; rc = aio_sock_getevents(queue, max, completions););
  n = rc;
  ...
  if (n < 0) {
    ...
    printf
      ("handle_completions: aio_sock_getevents returns < 0 when it should not\n");
    exit(1);
  } else if (n == 0) {
