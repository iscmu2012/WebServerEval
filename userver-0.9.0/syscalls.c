...
print_close_info()
{
  ...
  for (i = 0; i < num_sys_close; i++) {
    printf("%8d %8d %8d %8d %8d %8d %20ld\n",
      ...
      close_info[i].max_sd,
      close_info[i].max_open, close_info[i].open, close_info[i].time);
  }
}
#endif /* MAXOPENFD */
...
close_file(int fd)
{
  ...
  TRACE(EVT_CLOSE_FILE, trace_fd = fd; rc = close(fd););
  return rc;
}
...
close_sock(int sd)
{
  ...
  TRACE(EVT_CLOSE_SOCK, trace_fd = sd; rc = close(sd); save_errno = errno;);
