...
should_enable_new_conns(void)
{
  ...
  if (options.auto_accept) {
    ...
    PRINT_TIME(NOFD, &tnow, &tprev,
      "should_enable_new_conns: thold high = %d", options.ecb_threshold_high);
    PRINT_TIME(NOFD, &tnow, &tprev,
      "should_enable_new_conns: num_idle = %d " "free_fd_thold = %d\n",
...
conns_off_if_needed(void)
{
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "conns_off_if_needed: new_connections_on = %d", new_connections_on);
  ...
  switch (options.get_connections) {
    ...
    case OPT_CONN_WITH_SEND_EVTS:
      ...
      if (options.auto_accept) {
        ...
        /*       can be a bunch of accepted connections in the ecb buffer */
        /*       who already have fd's assigned. */
        if ((new_connections_on) && (free_slots <= options.ecb_threshold_low)) {
          PRINT_TIME(NOFD, &tnow, &tprev,
  ...
  switch (options.conn_on) {
      ...
      for (i = sock_listener_min; i <= sock_listener_max; i++) {
        if (sock_is_listener(i)) {
          TRACE(EVT_UNBLOCK_CONNS,
            trace_fd = i; interest_set_change(i, ISET_READABLE); rc = i;);
        }
      }
    ...
    case OPT_CONN_WITH_SEND_SELECT:
      for (i = sock_listener_min; i <= sock_listener_max; i++) {
        if (sock_is_listener(i)) {
          TRACE(EVT_UNBLOCK_CONNS,
            trace_fd = i; FD_SET(i, (fd_set *) & send_readfds); rc = i;);
        }
      }
      break;
#endif /* SEND */
    ...
    case OPT_CONN_WITH_SIGIO:
      ...
      TRACE(EVT_UNBLOCK_CONNS,
        ...
        unblock_sigio(); rc = sock_listener_max;);
      break;
    ...
    case OPT_CONN_WITH_SEND_EVTS:
      if (options.auto_accept) {
        ...
        TRACE(EVT_UNBLOCK_CONNS,
          trace_fd = sock_listener_min;
          unblock_auto_accept_delivery(); rc = sock_listener_max;);
      } else {
        TRACE(EVT_UNBLOCK_CONNS,
          /* trace_fd and rc are just placeholders here */
          trace_fd = sock_listener_min;
          unblock_send_events(); rc = sock_listener_max;);
      }
      break;
#endif /* SEND */
  ...
  switch (options.conn_off) {
      ...
      for (i = sock_listener_min; i <= sock_listener_max; i++) {
        if (sock_is_listener(i)) {
          TRACE(EVT_BLOCK_CONNS,
            trace_fd = i; interest_set_change(i, ISET_NOT_READABLE); rc = i;);
        }
      }
      break;
    ...
    case OPT_CONN_WITH_SEND_SELECT:
      for (i = sock_listener_min; i <= sock_listener_max; i++) {
        if (sock_is_listener(i)) {
          TRACE(EVT_BLOCK_CONNS,
            trace_fd = i; FD_CLR(i, (fd_set *) & send_readfds); rc = i;);
        }
      }
      break;
#endif /* SEND */
    ...
    case OPT_CONN_WITH_SIGIO:
      /* trace_fd and rc are just placeholders here */
      TRACE(EVT_BLOCK_CONNS,
        trace_fd = sock_listener_min; block_sigio(); rc = sock_listener_max;);
      break;
    ...
    case OPT_CONN_WITH_SEND_EVTS:
      if (options.auto_accept) {
        ...
        TRACE(EVT_BLOCK_CONNS,
          trace_fd = sock_listener_min;
          block_auto_accept_delivery(); rc = sock_listener_max;);
      } else {
        PRINT_TIME(NOFD, &tnow, &tprev,
        ...
        TRACE(EVT_BLOCK_CONNS,
          trace_fd = sock_listener_min;
          block_send_events(); rc = sock_listener_max;);
      }
      break;
#endif /* SEND */
