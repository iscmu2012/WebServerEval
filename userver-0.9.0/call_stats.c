...
#else
  call_stats =
    alloc_aligned(ALLOC_NOALIGN, (sizeof(call_data) * (buf_size)),
    ALLOC_MALLOC, "call_stats array");
#endif /* OLDWAY */

  if (call_stats == NULL) {
    fprintf(stderr,
      "call_stats_init: malloc failed to allocate call_stats array\n");
    exit(1);
  }
...
call_stats_request(int req_num, char *uri)
{
  ...
  if (index < buf_size) {
    ...
    call_stats[index].time =
      ((double) cur.tv_sec * 1000000.0) + (double) cur.tv_usec;
  }
...
call_stats_print()
{
  ...
  for (i = 1; i < MIN(add_calls + 1, buf_size); i++) {
    ...
    if (call_stats[i].size == 0) {
      ...
      percent =
        (int) (double) call_stats[i].bytes_sent * 100.0 /
        (double) call_stats[i].size;
    }
