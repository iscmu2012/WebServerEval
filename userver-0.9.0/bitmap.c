...
alloc_bitmap(size_t size)
{
  ...
  numbytes =
    (size_t) offsetof(bitmap_t, map) + (ROUND_UP(size, WORD_BITS) / 8);
  if (numbytes < (size_t) offsetof(bitmap_t, map)) {
    goto overflow;
  }

  printf
    ("alloc_bitmap: size = %lu WORD_BITS = %lu ROUND_UP = %lu roundup/8 = %lu, numbytes = %lu\n",
    (unsigned long) size, (unsigned long) WORD_BITS,
    (unsigned long) ROUND_UP(size, WORD_BITS), (unsigned long) (ROUND_UP(size,
        WORD_BITS) / 8), (unsigned long) numbytes);
