...
fork_servers(int numprocs)
{
...
#else
    sprintf(myfile, "userver-%d.log",
      my_proc_id + numprocs * options.server_id);
#endif /* OLDWAY */
  ...
  if (options.get_connections == OPT_CONN_WITH_SEND_EVTS) {
    ...
    PRINT_TIME(NOFD, &tnow, &tprev,
      "fork_servers: queue = 0x%x notify = 0x%x", ecb->queue, ecb->notify);
  }
...
#ifdef HAVE_EPOLL
  if (options.use_epoll || options.epoll_edge_triggered || options.use_epoll2) {
...
pid_t
fork_app_server(const char *exec_string)
{
...
static int
configure_appserver(int proc_id)
{
  app_t *app;
  ...
  for (i = 0; i < options.app_count; i++) {
    ...
    while (appserver) {
      ...
      if (port_str == NULL) {
        printf
          ("configure_appserver:No port specified. Failed to configure appserver: %s,%s\n",
          app->uri, path);
        return -1;
      }
      *port_str++ = '\0';
      ...
      if (appserver->startinfo) {
        ...
        exec_str =
          (char *) calloc(strlen(start->exec_string) + strlen(serverpath) + 3,
          sizeof(char));
...
#ifdef OLDWAY
        printf
          ("configure_appserver:No startinfo. Failed to configure appserver: %s,%s:%s\n",
...
start_appservers()
{
  ...
  while (start != NULL) {
    ...
    if (rc < 0) {
      printf
        ("start_appservers: unable to set processor affinity mask for %s appserver\n",
        start->path);
    ...
    if (rc < 0) {
      printf
        ("start_appservers: unable to get processor affinity mask for %s appserver\n",
        start->path);
      printf("start_appservers: call returns %d errno = %d\n", rc, errno);
...
#else
    printf
      ("start_appservers: SCHED_AFFINITY not defined. ignoring appserv_cpu_mask\n");
#endif /* HAVE_SCHED_AFFINITY */
...
set_userver_cpu_affinity()
{
  ...
  if (rc < 0) {
    printf
      ("set_userver_cpu_affinity: unable to set processor affinity mask\n");
  ...
  if (rc < 0) {
    printf
      ("set_userver_cpu_affinity: unable to get processor affinity mask\n");
    printf("set_userver_cpu_affinity: call returns %d errno = %d\n", rc,
      errno);
    perror("unable to sched_getaffinity");
    exit(1);
