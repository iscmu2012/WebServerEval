...
send_do_io(int sd, struct info *ip, int events)
{
  ...
  PRINT_TIME(sd, &tnow, &tprev,
    "send_do_io: fsm_state = %s events = 0x%x", fsm_state_string(sd), events);

#ifdef SEND
  PRINT_TIME(sd, &tnow, &tprev, "send_do_io: fsm_state = %d", ip->fsm_state);
  ...
  if ((!event_processed) && (info_map_valid(sd)) && (events & POLLERR)) {
    PRINT_TIME(sd, &tnow, &tprev, "send_do_io: POLLERR calling do_close");
  ...
  if ((!event_processed) && (info_map_valid(sd)) && (events & POLLFIN)) {
    if (ip->fsm_state == FSM_READING_NEXT_REQUEST) {
      PRINT_TIME(sd, &tnow, &tprev, "send_do_io: got POLLFIN close normal");
      num_close_send_normal++;
    } else {
      PRINT_TIME(sd, &tnow, &tprev, "send_do_io: got POLLFIN close early");
  ...
  if ((!event_processed) && (info_map_valid(sd)) &&
    ((ip->fsm_state == FSM_READING_REQUEST) ||
      (ip->fsm_state == FSM_READING_NEXT_REQUEST)) && (events & POLLIN)) {
    PRINT_TIME(sd, &tnow, &tprev, "send_do_io: calling socket_readable");
  ...
  if ((!event_processed) && (info_map_valid(sd)) &&
    ...
    PRINT_TIME(sd, &tnow, &tprev, "send_do_io: calling socket_writable");
