...
send_reply(int sd, struct rep *rep)
{
  ...
  if (
    ...
    ip->type == INFO_CLIENT && ip->req.method != HTTP_RETURN_ERROR) {
#ifdef HAVE_SENDFILE
...
do_aio_write_cached(struct info *ip)
{
  ...
  TRACE(EVT_AIO_SOCK_WRITEV,
    trace_fd = sd; rc = aio_sock_writev(sd, aio_vec, vec_count););
  save_errno = errno;
  DEBG(MSG_WRITEV, "rc = %d\n", rc);


  PRINT_TIME(sd, &tnow, &tprev, "do_aio_write_cached: returning rc = %d", rc);
  return (rc);
} /* do_aio_write_cached */
#endif /* HAVE_AIO_LAYER */
...
post_sock_writev(int sd, int nwritten, int err)
{
  ...
  if (nwritten < 0) {
    ...
    PRINT_TIME(sd, &tnow, &tprev,
      "post_sock_writev: write failed -  returning -1");
    return -1;
  }
...
do_write_cached(struct info *ip)
{
  ...
  do {
     ...
     */
    PRINT_TIME(sd, &tnow, &tprev,
      "do_write_cached: total_len = %d nwritten = %d",
      rep->total_len, rep->nwritten);
    PRINT_TIME(sd, &tnow, &tprev,
    ...
    if (nwritten < bytes_to_write) {
      ...
      PRINT_TIME(sd, &tnow, &tprev, "do_write_cached: returning %d",
        nwritten);
      return (nwritten);
    }
