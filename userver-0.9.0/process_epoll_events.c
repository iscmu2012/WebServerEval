...
int do_epoll_event_et(int n, struct epoll_event *evt, int *did_something,
  int *did_accept);
    ...
    case OPT_PROCESS_SDS_DOWN:
      for (i = num_fds - 1; i >= 0; i--) {
        ...
        if (options.epoll_edge_triggered) {
          n =
            do_epoll_event_et(n, &epoll_results[i], &handled_event,
            &did_accept);
        } else {
    ...
    case OPT_PROCESS_SDS_WRITES_FIRST_DOWN:
      ...
      for (i = num_fds - 1; n > 0 && i >= 0; i--) {
        if (epoll_is_writable(epoll_results[i].events)) {
          ...
          PRINT_TIME(sd, &tnow, &tprev, "process_epoll_events: do_writable ");
          do_writable(sd);
      ...
      for (i = num_fds - 1; n > 0 && i >= 0; i--) {
        ...
        if (epoll_is_readable(epoll_results[i].events)) {
          --n;
          sd = epoll_results[i].data.fd;
          PRINT_TIME(sd, &tnow, &tprev, "process_epoll_events: do_readable ");
          do_readable(sd);
          do_extra_accepts(n, options.extra_freq);
        }
      } /* for */
...
do_epoll_event(int n, struct epoll_event *evt, int *did_something)
{
  ...
  if (epoll_is_hungup(evt->events)) {
    ...
    PRINT_TIME(sd, &tnow, &tprev,
      "do_epoll_event: epoll got EPOLLHUP or EPOLLERR");
    ...
    if (fsm_state_reading(sd)) {
      ...
      *did_something = 1;
      ...
      PRINT_TIME(sd, &tnow, &tprev,
        "do_epoll_event: epoll got EPOLLHUP or EPOLLERR and did not read or write");
      switch (get_fsm_state(sd)) {
        ...
        case FSM_FCGI_REQUEST_QUEUED:
          PRINT_TIME(sd, &tnow, &tprev,
            "do_epoll_event: converting to hupped");
  ...
  int *did_something, int *did_accept)
{
  ...
  if (sock_is_listener(sd)) {
    ...
    *did_accept = 1;
    ...
    if (info_map_valid(sd)) {
      PRINT_TIME(sd, &tnow, &tprev,
        "do_epoll_event_et: fsm_state = %s ip->sd_state = 0x%x",
      ...
      PRINT_TIME(sd, &tnow, &tprev,
        "do_epoll_event_et: info_map not valid 1\n");
    }
    ...
    if (info_map_valid(sd)) {
      ...
      PRINT_TIME(sd, &tnow, &tprev,
        "do_epoll_event_et: info_map not valid 2\n");
    }
