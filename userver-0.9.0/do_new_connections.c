...
do_new_connections(int listen_sd, int called_from)
{
  ...
  if (num_idle <= options.free_fd_thold) {
    ...
    PRINT_TIME(NOFD, &tnow, &tprev,
      "do_new_connections: no idle - returning");
    DEBG(MSG_CONN,
  ...
  switch (options.do_multiaccept) {
...
#ifdef MACCEPT
      TRACE(EVT_MACCEPT, trace_fd = 0; rc = multiaccept(listen_sd););
      consecutive = rc;
#else
    ...
    case OPT_MULTIACCEPT_BY_REPEATING:
      ...
      if (listen_sd == SOCK_LISTENER_USE_ALL) {
        for (i = sock_listener_min; i <= sock_listener_max; i++) {
          if (info_listener[i].sd != -1) {
            TRACE(EVT_REPEAT_ACCEPT,
              trace_fd = 0; rc = repeat_accept(info_listener[i].sd););
            consecutive = rc;
        ...
        TRACE(EVT_REPEAT_ACCEPT,
          trace_fd = 0; rc = repeat_accept(listen_sd););
        consecutive = rc;
      }
