...
// #endif
#ifdef UMEMREDUCTION
  char *tmp;                    //points to stack buffer, unless heap space is needed
  int tmp_len;
  int tmp_allocated;            //true if heap space has been allocated
  char *uri;                    //points into read buf, unless heap space is needed
  int uri_len;
  int uri_allocated;            //true if heap space has been allocated
  char *cookie;                 //points into read buf, unless heap space is needed
  int cookie_allocated;         //true if heap space has been allocated
#else
  int tmp_len;
  /*
