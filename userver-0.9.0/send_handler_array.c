...
int send_handle_new_conn(const struct event *evt, int listen_sd, int sd,
  int process);
...
free_ecb_slots_in_buf(int buf_index)
{
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "free_ecb_slots_in_buf: buf = %d returning %d", buf_index, free_slots);
  return (free_slots);
} /* free_ecb_slots */
...
{
  PRINT_TIME(NOFD, &tnow, &tprev,
    "block_auto_accept_delivery: before queue = 0x%x", ecb->queue);
  assert(ecb);
  /* This was the blocking the wrong thing */
  /* ecb->queue = evt_queue & (~EVT_IPACCEPT); */
  ecb->queue = ecb->queue & (~AUTO_ACCEPT_GRP);
  PRINT_TIME(NOFD, &tnow, &tprev,
    "block_auto_accept_delivery: after  queue = 0x%x", ecb->queue);
}
...
unblock_auto_accept_delivery(void)
{
  assert(ecb);
  PRINT_TIME(NOFD, &tnow, &tprev,
    "unblock_auto_accept_delivery: before queue = 0x%x", ecb->queue);
  /* This was the blocking the wrong thing and was just plain wrong */
  /* ecb->queue = evt_queue | (~EVT_IPACCEPT); */
  ecb->queue = ecb->queue | AUTO_ACCEPT_GRP;
  PRINT_TIME(NOFD, &tnow, &tprev,
    "unblock_auto_accept_delivery: after  queue = 0x%x", ecb->queue);
}
...
bufs_empty(void)
{
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "bufs_empty: buf_index = %d returning %d", buf_index, all_bufs_empty);
...
buf_is_empty(int buf_index)
{
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "buf_is_empty: buf_index = %d returning %d", buf_index, buf_is_empty);
  return (buf_is_empty);
}
...
/*----------------------------------------------------------------------*/
int
send_handle_new_conn(const struct event *evt, int listen_sd, int sd,
  int process_sd)
{
  int rc = 0;
  int i = 0;
  ...
  if (num_idle > 0) {
    if (!options.auto_accept) {
      ...
      TRACE(EVT_AUTO_ACCEPT, trace_fd = sd; rc = sd;);
      ...
      if (options.accepts_only) {
        ...
        PRINT_TIME(sd, &tnow, &tprev, "send_handle_new_conn: calling close");
        close_sock(sd);
        num_idle++;
        set_fsm_state(ip, FSM_CLOSED);
        PRINT_TIME(sd, &tnow, &tprev, "send_handle_new_conn: closed");
        ip->sd = -1;
