...
void check_and_do_read_write(int *n, int sd, fd_set * readable,
  fd_set * writable);
  ...
  if (ip->skip > 0) {
    if (ip->skip_info < ip->skip) {
      ...
      PRINT_TIME(sd, &tnow, &tprev,
        "do_writable: skipping write ip->skip = %d ip->skip_info %d",
      ...
      ip->skip_info = 0;
      PRINT_TIME(sd, &tnow, &tprev,
        "do_writable: resetting skip_info ip->skip_info = %d", ip->skip_info);
    }
  }
