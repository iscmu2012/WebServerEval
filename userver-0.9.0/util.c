  ...
  const char *print_str, alloc_track_t ** caller_track)
{
  ...
  ldbg
    ("alloc_aligned_track: %s alignment = %u (0x%x) size = %zu flags = 0x%x\n",
  ...
  switch (flags & 0xFFFF) {
    ...
    case ALLOC_MMAP_FILE:
      ...
      if (ftruncate(track->mmap_fd, size + alignment) < 0) {
        ...
        printf
          ("alloc_aligned_track: %s ftruncate(size = %zu) failed errno = %d\n",
  ...
  if (start == NULL) {
    ...
    printf("alloc_aligned_track: %s %s failed errno = %d\n",
      print_str,
      (flags & (ALLOC_MMAP | ALLOC_MMAP_FILE)) ? "mmap" : "malloc", errno);
    exit(1);
  } else {
  ...
  if (alignment != 0) {
    ...
    if (((uintptr_t) adjusted & alignment_mask) != (uintptr_t) adjusted) {
      adjusted =
        (void *) (((uintptr_t) ((uintptr_t) start +
  ...
  printf
    ("alloc_aligned_track: alloced = %zu (%zu MB) total alloced = %zu (%zu MB)\n",
...
get_actual_sock_status(int sd, long timeout, char which)
{
  ...
  if (options.use_epoll) {
    ...
    if (rc < 0) {
      printf
        ("get_actual_sock_status: epoll_wait FAILED on %d with rc = %d errno = %d\n",
        sd, rc, save_errno);
    } else {
...
/* Parameters:
 * str      - typically indicating where the function has been called from.
 * sd       - socket descriptor being used for the output
 * file_fd  - the file the data is coming from
 * offset   - where in the file we are starting
...
print_sendfile_buf(char *str, int sd, int file_fd, off_t offset, int len)
{
  ...
  printf
    ("print_sendfile_buf: from %s sd = %d file_fd = %d offset = %llu len = %d\n",
    str, sd, file_fd, (unsigned long long) offset, len);
  ...
  /* Just mmap the whole file and we will try to print the subset being sent */
  buf = (char *) mmap(0, statbuf.st_size, PROT_READ, MAP_PRIVATE, file_fd, 0);
  if (buf == MAP_FAILED) {
    perror("mmap");
  ...
  assert(len <= statbuf.st_size);
  assert(len + offset <= statbuf.st_size);

  printf("print_sendfile_buf: contents: --start--\n");
  for (i = offset; i < len + offset; i++) {
    assert(i <= statbuf.st_size);
    printf("%c", buf[i]);
  }
  printf("\n---end---\n");
