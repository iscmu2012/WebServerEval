...
socket_new_conn(int listen_sd)
{
  ...
  if (options.use_aio_accept) {
    ...
    if (options.aio_read_before_accept) {
      ...
      PRINT_TIME(listen_sd, &tnow, &tprev, "socket_new_conn: "
        "aio_sock_read_accept ip = %p", ip);
      TRACE(EVT_AIO_SOCK_READ_ACCEPT,
        trace_fd = listen_sd;
        rc = aio_sock_read_accept(listen_sd, ip->req.buf,
          ip->req.buf_size - 1, ip->req.read_reg_handle); err = errno;);
      ...
      PRINT_TIME(listen_sd, &tnow, &tprev,
        "socket_new_conn: aio_sock_accept");
      TRACE(EVT_AIO_SOCK_ACCEPT, trace_fd = listen_sd;
        rc = aio_sock_accept(listen_sd); err = errno;);
    }
#else
    ...
    if (options.use_accept_send) {
      ...
      TRACE(EVT_ACCEPT,
        ...
        ip->addr.len = sizeof(ip->addr.u);
        errno = 0;
        rc = accept(listen_sd, &ip->addr.u.sa, &ip->addr.len); err = errno;);
    }
  ...
  if (is_ssl_listener(listen_sd) && sd > 0) {
    ...
    TRACE(EVT_ACCEPT,
      trace_fd = listen_sd;
      ip->addr.len = sizeof(ip->addr.u); client_ssl = do_ssl_accept(sd);
  ...
  PRINT_TIME(listen_sd, &tnow, &tprev,
    "socket_new_conn: sd = %d returning %d", sd, x);
  return (x);
}
...
socket_setup(int listen_sd, int sd)
{
  ...
  /* good for debugging... */
#ifdef UMEMREDUCTION
  //uri could be pointing to NULL at this point
  if (ip->req.uri) {
    ip->req.uri[0] = '\0';
  }
#else
...
#ifdef UMEMREDUCTION
  //if a root document directory has been specified it will be dealt with at
  //the time of reading
  if (options.doc_root[0] != '\0') {
    // TODO
    // Need to be able to prepend the doc root to the uri.
    // This may be non trival in the reduce memory case because
    // we try not to have copies of things around.
    printf("UMEMREDUCTION can't be used with --doc-root option yet\n");
    exit(1);
  }
#else
  ...
  PRINT_TIME(sd, &tnow, &tprev, "socket_setup: lru_access on ip->sd = %d",
    ip->sd);
  lru_access(ip->sd);
...
multiaccept(int listen_sd)
{
  ...
  TRACE(EVT_MACCEPT,
    ...
    rc = socketcall(SYS_MACCEPT, &syscall_args); err = errno;);
...
post_sock_accept(int listen_sd, struct info *ip, int sd, int err)
{
  ...
  if (sd > max_sock_sd) {
    print_all();
    printf
      ("EXITING from  post_sock_accept: new sd = %d is > max_sock_sd = %d\n",
      sd, max_sock_sd);
    exit(1);
  }
  ...
  if (listen_sd != SOCK_LISTENER_UNKNOWN) {
    ...
    PRINT_TIME(sd, &tnow, &tprev, "post_sock_accept: listen_sd = %d "
      "cur_conns = %d", listen_sd, ilp->cur_conns);
  } else {
    /* TODO */
...
set_client_sockopts(int listen_sd, int sd)
{
  ...
  if (((options.get_connections == OPT_CONN_WITH_SEND_EVTS) ||
    ...
    PRINT_TIME(sd, &tnow, &tprev,
      "set_client_sockopts: O_NONBLOCK | O_ASYNC");
