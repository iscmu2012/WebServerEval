...
reply_status_fill(struct info *ip, int status, char *str, int http_version)
{
  ...
  switch (status) {
    ...
    case HTTP_NOT_MODIFIED:
      ...
      gmt_time = gmtime(&now);
      len =
        strftime(tmp_buf, STATUS_BUF_LEN, "%a, %d %b %Y %T GMT", gmt_time);
      assert(len != 0);

      if (http_version == 1) {
        assert(HTTP_NOT_MODIFIED_STRLEN < ip->rep.buf_size);
        memcpy(ip->rep.buf, HTTP_11_NOT_MODIFIED_STR,
          HTTP_NOT_MODIFIED_STRLEN);
      } else {
        assert(HTTP_NOT_MODIFIED_STRLEN < ip->rep.buf_size);
        memcpy(ip->rep.buf, HTTP_10_NOT_MODIFIED_STR,
          HTTP_NOT_MODIFIED_STRLEN);
      }
    ...
    case HTTP_SERVICE_UNAVAILABLE:
      ...
      len = snprintf(ip->rep.buf, ip->rep.buf_size,
        HTTP_HDR_STR_ID_LEN "%s", http_version, status,
        HTTP_SERVICE_UNAVAILABLE_IDENT, server_ident, (unsigned long) len,
        tmp_buf);
      assert(len > 0);
      assert(len < ip->rep.buf_size);
    ...
    case HTTP_NOT_IMPLEMENTED:
      ...
      len = snprintf(ip->rep.buf, ip->rep.buf_size,
        HTTP_HDR_STR_ID_LEN "%s", http_version, status,
        HTTP_NOT_IMPLEMENTED_IDENT, server_ident, (unsigned long) len,
        tmp_buf);
      assert(len > 0);
      assert(len < ip->rep.buf_size);
  ...
  PRINT_TIME(ip->sd, &tnow, &tprev,
    "reply_status_fill: rep.buf = %s", ip->rep.buf);
  PRINT_TIME(ip->sd, &tnow, &tprev,
    "reply_status_fill: rep.cur = %p rep.end = %p", ip->rep.cur, ip->rep.end);
