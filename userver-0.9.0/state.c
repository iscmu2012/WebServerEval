...
fsm_state_reading(int sd)
{
  ...
  if ((fsm_state == FSM_READING_REQUEST) ||
    (fsm_state == FSM_READING_NEXT_REQUEST) || (fsm_state == FSM_LISTENING)) {
    retval = 1;
  } else {
    retval = 0;
  }
...
convert_to_hupped(struct info *client_ip)
{
  ...
  client_ip->got_hup = 1;
  interest_set_change(client_ip->sd,
...
sd_state_not_readable(struct info *ip)
{
  if ((ip->sd_state & EPOLLHUP) || (ip->sd_state & EPOLLERR)) {
    PRINT_TIME(ip->sd, &tnow, &tprev,
      "sd_state_not_readable: but have HUP/ERR sd_state now = 0x%x",
      ip->sd_state);
  } else {
    ip->sd_state &= ~(EPOLLIN);
    ldbg("sd_state_not_readable: sd_state now = 0x%x\n", ip->sd_state);
    PRINT_TIME(ip->sd, &tnow, &tprev,
      "sd_state_not_readable: sd_state now = 0x%x", ip->sd_state);
  }
}
...
unblock_hupped_client(struct info *client_ip)
{
  ...
  interest_set_change(client_sd,
    ISET_NOT_WRITABLE | ISET_READABLE | ISET_INIT);
