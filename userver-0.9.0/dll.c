...
#else
  list =
    (struct dll_list *) alloc_aligned(ALLOC_NOALIGN, sizeof(struct dll_list),
...
#ifdef OLDWAY
  list->free_head =
    (struct dll_element *) malloc(size * sizeof(struct dll_element));
#else
  list->free_head = (struct dll_element *) alloc_aligned(ALLOC_NOALIGN,
    (size * sizeof(struct dll_element)), ALLOC_MALLOC,
    "dll init_list: free head");
#endif /* OLDWAY */
  if (list == 0) {
    ...
    if (client != NULL) {
      ...
      if ((client->done_write == 1) && (client->rep.nwritten > 0) &&
        ...
        if (options.use_sendfile) {
          ...
          res =
            getsockopt(retval->key_sd, SOL_SOCKET, 35, &my_stats, &opt_res);
          if (res != 0) {
            perror("get sock opts:");
            printf
              ("ERROR: using sendfile but need way to check if buffer can be reused\n");
            assert(0);
          }
