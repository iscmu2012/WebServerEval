/*
    ...
    This file is Copyright (C) 2004-2011  Tim Brecht and Craig Barkhouse
...
/* size    - total bytes in the file
 * data    - string to use instead of default
 * datalen - length of data?
 * header  - 0 means no header, 1 means add header
 */

int
create_doc(char *filename, long size, char *data, long datalen, int header)
{
  int count = 0;
  ...
  while (size > 0) {
    ...
    i = cur_pos;
    count = 0;
    while (count < strlen(data) && size > 0) {
