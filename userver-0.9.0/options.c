...
options_print(void)
{
  ...
  printf("\n");
  print_cpuset_values("options.appserv_cpu_mask = ",
...
options_print_xml(FILE * destf)
{
  ...
  fprintf(destf, "    <opt name='get_connections'> %d </opt>\n",
    options.get_connections);
  fprintf(destf,
    "    <opt name='get_connections_in_server_loop'> %d </opt>\n",
    options.get_connections_in_server_loop);
  ...
  fprintf(destf,
    "    <opt name='num_dyn_buffers_per_app_server'> %d </opt>\n",
    options.num_dyn_buffers_per_appserver);
  ...
  for (i = 0; i < options.app_count; i++) {
    ...
    fprintf(destf, "    <app index='%d' uri='%s' req_type='%s'>\n", i,
      app->uri, req_type_str(app->req_type));
    appserver = app->server;
    for (j = 0; appserver != NULL; j++, appserver = appserver->next)
      fprintf(destf, "      <appserver index='%d' path='%s'/>", i,
        appserver->path);
  }
  fprintf(destf, "  </category>\n\n");
