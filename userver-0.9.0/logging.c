...
log_request(struct info *ip)
{
  ...
  char timestr[32];
  char *end = NULL;
  int qs = 0;                   /* Is this a query string ? */
  ...
  fprintf(log, "%s - - %s \"%s\" %d %d \n",
    client_address, timestr, ip->req.uri,
    ip->rep.http_status, ip->rep.nwritten);
#else
  qs = ip->req.query_string != NULL;
  fprintf(log, "%s - - %s \"%s%s%s\" %d %d ",
    client_address, timestr, ip->req.uri, (qs ? "?" : ""),
    (qs ? ip->req.query_string : ""), ip->rep.http_status, ip->rep.nwritten);

  if (range_request(ip)) {
    fprintf(log, "%d %d", (int) ip->req.range_start, (int) ip->req.range_end);
  }
