...
static struct option long_options[] = {
  ...
  {"aio-initial-prepost-only", no_argument, &options.aio_initial_prepost_only,
      1},
...
process_args(int argc, char **argv)
{
  ...
  char *uri_arg, *type_arg, *path_arg, *port_arg, *count_arg, *exec_arg,
    *mask_arg;
  ...
  while (1) {
    ...
    ch = getopt(argc, argv,
      "ABCD:EFGHI:J:K:L:M:NO:PQ:R:ST:UVW:XYZ"
      "ab:c:d:e:f:ghijkl:m:no:p:q:rst:uvwxyz:?-" "123");
    if (ch == EOF) {
      done = 1;
    }
#endif
    ...
    if (done) {
      if (optind < argc) {
        printf
          ("Unable to parse command line (error occurred around option %d)!\n",
          optind);
        exit(1);
      } else {
        break;
    ...
    switch (ch) {
      ...
      case 0:
        ...
        if (flag == (int *) &options.is_cfg_filename) {
          ...
          if (options.aio_complq_count == 1) {
            ...
            strncpy(options.aio_completion_order,
              OPT_AIO_ONE_QUEUE_COMPLETION_ORDER, OPT_AIO_COMPL_ORDER_LEN);
          } else {
          ...
          if ((options.encrypt_percent < 0)
            || (options.encrypt_percent > 100)) {
            printf
              ("Invalid encryption percentage; must be >= 0 and <= 100\n");
          ...
          if (type_arg == NULL) {
            ...
            if (path_arg == NULL) {
              printf("No application path specified!\n");
              printf
                ("Expected format for FastCGI-sharedmem: uri,type,path[,count]\n");
              exit(1);
            }
            req_type = REQ_FASTCGI;
...
#else
                printf
                  ("CPU mask/affinity was not enabled during compilation\n");
                exit(1);
#endif /* HAVE_SCHED_AFFINITY */
              }
            }
            if (*exec_arg == '"')
              exec_arg++; /*Strip the quotes if present */
            exec_strlen = strlen(exec_arg);
            if (exec_arg[exec_strlen - 1] == '"')
              exec_arg[--exec_strlen] = '\0';
          }
          count_arg = strchr(path_arg, ',');
          if (count_arg != NULL) {
            *count_arg++ = '\0';
          ...
          if (count_arg != NULL) {
            ...
            if (count > options.appserver_count) {
              printf
                ("appserver_startupinfo_count: %d more than appserver_count: %d\n",
            ...
            for (i = 0; i < count; i++) {
              sprintf(serverpath, "%s:%d", path_arg,
                ntohs(addr.u.sa_in.sin_port) + i);
#ifdef HAVE_SCHED_AFFINITY
              if (app_add_startinfo(serverpath, exec_arg,
                  appserv_cpu_mask) < 0) {
#else
            ...
            if (options.appserver_count > 1) {
              printf
                ("appserver_startinfo_count: 1 more than appserver_count: %d\n",
...
#if !defined(SUNOS) && !defined(__CYGWIN__) && !defined(__linux__)
      case 'I':
      ...
      case 'J':
        printf
          ("Changing socket buffer low water mark not permitted on this OS\n");
        exit(1);
        break;
#endif /* ! SUNOS */
      ...
      case 'f':
        ...
        if (max_fds < 10) {
          printf
            ("A minimum number of fds are needed - setting max_fds to 10\n");
          max_fds = 10;
        }
...
check_options()
{
  ...
  if (options.victim_skip != 0) {
    if (options.victim_str[0] == '\0') {
      printf
        ("If specifying --victim-skip you must also specify a victim string using --victim\n");
      exit(1);
    }
  }
  ...
  if (options.ignore_fd_setsize && need_fd_setsize()) {
    ...
    printf
      ("Trying to avoid using FD_SETSIZE but the options chosen require it\n");
  ...
  if (maxconns > FD_SETSIZE) {
    printf
      ("Trying to allow more open connections than permitted by FD_SETSIZE\n");
  ...
  if (options.use_epoll2 && options.appserver_count > 0) {
    printf("ERROR IN OPTION SELECTION\n");
    printf
      ("Error: epoll2 is not currently implemented for dynamic workloads\n");
    exit(1);
  }
  ...
  if (options.epoll_edge_triggered && options.appserver_count > 0) {
    printf("ERROR IN OPTION SELECTION\n");
    printf
      ("Error: edge-triggered epoll is not currently implemented for dynamic workloads\n");
    exit(1);
  }
  ...
  if ((options.blocking_sendfile) && (!options.use_sendfile)) {
    printf("ERROR IN OPTION SELECTION\n");
    printf
      ("Error: Must specify --use-sendfile whenever --blocking-sendfile is specified\n");
    exit(-1);
  }
  ...
  if (app_match_startinfo_all() < 0) {
    printf("ERROR IN OPTION SELECTION\n");
    printf
      ("Error: Application servers must be defined using --app before they can be started!\n");
    exit(1);
  }
