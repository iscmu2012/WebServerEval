 ...
 */
static int
error(user_req_t * ip, char *message)
{
...
static CAD_t *
parse_GET_cookies(user_req_t * ip)
{
  int uid, last_ad;
  CAD_t *CADp;
  char *tmp;
...
out:
  ...
  tmp = buf;
  tmp +=
    sprintf(tmp, "%10d %10ld %10d %5d %2d %2d %10d %-60.60s %10d %10d\n",
    atoi(count_buf), time(NULL), this_pid, dir, post_class, num, client,
    CADp->ad_filename, this_pid, CADp->user_id + 10000);
  ...
  CADp->reply_cookies_len =
    sprintf(CADp->reply_cookies, "my_cookie=%u", 10000 + CADp->user_id);
  return 0;
...
static int
my_send(int sock, char *buf, int total_len, int flags)
{
...
static int
read_user_personality(user_req_t * ip)
{
  int file;
  int ret, len, err = -2;
  char *buf = NULL, *tmp;
...
static inline int
find_ad(int user_id, int last_ad, int *weight_p)
{
  int adid, dem;
...
static void
open_postlog_file(user_req_t * ip)
{
...
static int
init_postlog_file(user_req_t * ip)
{
  char count_buf[11];
...
static int
do_reset(user_req_t * ip, char *query)
{
...
static int
do_command(user_req_t * ip, char *query)
{
...
static inline int
custom_ad_rotate(user_req_t * ip, CAD_t * CADp)
{
...
error:
  CADp->reply_cookies_len = sprintf(CADp->reply_cookies,
    "found_cookie=Ad_id=%d&Ad_weight=%d&Expired=%d", adid, weight, expired);
...
static int inline
send_reply_head(user_req_t * ip, size_t body_size)
{
  int ret;
  char buf[1000];
  char *tmp, *head, *tail;
...
#define  PARSED_MODULE_NAME "spec_dyn_req"
  ...
  tmp += sprintf(tmp, REPLY_HEAD_TAIL, server_ident,
    /*IP(0), IP(1), IP(2), IP(3), */
    0, 0, 0, 0, PARSED_MODULE_NAME, ip->req.query_string);
  ...
  tmp =
    (char *) (((unsigned int) tmp + sizeof(int) - 1) & ~(sizeof(int) - 1));
  head = tmp;
...
static int
direct_send_file(user_req_t * ip)
{
  int ret = 0;
  off_t offset = 0;
