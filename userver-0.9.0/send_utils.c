...
dump_regs(struct evt_regs *r)
{
  ...
  if ((r != (struct evt_regs *) 0xffffffff) && (r != (struct evt_regs *) 0)) {
  ...
  switch (evt->type) {
    ...
    case EVT_IOREADY:
      printf("         # IO-ready event: fd: %i events: %08x tag: %p\n",
        evt->data.io.fd, evt->data.io.events, evt->data.io.tag);
    ...
    case EVT_MSG:
      printf("         # Message event  sender: %i data: %p len: %i\n",
        evt->event_id, evt->data.msg.data, evt->data.msg.len);
      break;
    ...
    case EVT_IPACCEPT:
      printf("         # IP Accept event: fd: %i addr: %s "
        ...
        ntohs(evt->data.ipa.sin.sin_port), (void *) evt->data.ipa.events);
      print_io_events(evt->data.io.events);
      break;
    ...
    case EVT_DISPATCH:
      printf("         # Dispatch event id: %i token: %p reason %08x\n",
        ...
        (void *) evt->data.dispatch.token, evt->data.dispatch.tag);
      break;
...
print_ecb_types(void)
{
  ...
  for (i = 0; i < ecb->event_list_size; ++i) {
    if (ecb->eventbuf[i].type != -1) {
      if (ecb->eventbuf[i].type >= 0 && ecb->eventbuf[i].type <= 1000) {
