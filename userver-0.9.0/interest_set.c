...
poll_event_change(int sd, unsigned int mask)
{
  ...
  PRINT_TIME(sd, &tnow, &tprev, "poll_event_change: sd = %d mask = 0x%x "
    "max_poll_fd = %d", sd, mask, max_poll_fd);
  ...
  PRINT_TIME(sd, &tnow, &tprev, "poll_event_change: sd = %d mask = 0x%x "
    "max_poll_entry = %d", sd, mask, max_poll_entry);
...
/* Done this way because nesting these macros doesn't work */
    ...
    TRACE(EVT_EPOLL_CTL,
      trace_fd = sd;
      rc = epoll_ctl(epoll_fd, epoll_op, sd, &ep_evt); save_errno = errno;);
        ...
        if (sock_is_listener(sd)) {
          if (options.epoll_modify_listeners) {
            PRINT_TIME(sd, &tnow, &tprev,
              "epoll_event_change: will change listener");
          } else {
            PRINT_TIME(sd, &tnow, &tprev,
              "epoll_event_change: will not change listener");
            return;
          }
        } else {
          PRINT_TIME(sd, &tnow, &tprev,
            "epoll_event_change: epoll2 no change on non listener");
          return;
        }
      }
    ...
    if (options.use_epoll_ctlv) {
      ...
      if (epoll_op == EPOLL_CTL_ADD) {
        PRINT_TIME(sd, &tnow, &tprev,
          "epoll_event_change: sd = %d op = 0x%x event = 0x%x " "adding", sd,
          epoll_op, ep_evt.events);
      } else if (epoll_op == EPOLL_CTL_MOD) {
        PRINT_TIME(sd, &tnow, &tprev,
          "epoll_event_change: sd = %d op = 0x%x event = 0x%x " "modifying",
          sd, epoll_op, ep_evt.events);
      } else {
        PRINT_TIME(sd, &tnow, &tprev,
          "epoll_event_change: sd = %d op = 0x%x event = 0x%x " "STRANGE OP",
          sd, epoll_op, ep_evt.events);
      }
...
interest_set_change(int sd, unsigned int mask)
{
  ...
  switch (set_method) {
    ...
    case DECLARE_WITH_EPOLL:
      PRINT_TIME(sd, &tnow, &tprev,
        "interest_set_change: epoll_fd = %d sd = %d " "mask = 0x%x", epoll_fd,
        sd, mask);
...
{
  return (event_set_is_readable(sd) || event_set_is_writable(sd));
}
...
#ifdef LOCAL_DEBUG
  void poll_array_verify(int before);
  poll_array_verify(BEFORE);
#endif /* LOCAL_DEBUG */

  for (i = 0; i < max_poll_entry; i++) {
    if (poll_array[i].fd == -1) {

      /* Skip over any entries that aren't needed/used anymore */
      while (poll_array[max_poll_entry - 1].fd == -1) {
        max_poll_entry--;
      }
      ...
      if (i < max_poll_entry) {
        /* Copy the last used entry in the array into the current position */
...
#if 0
  for (i = 0; i < max_poll_entry; i++) {
    if (poll_map[poll_array[i].fd] != i) {

      printf
        ("***** ERROR poll_array[%d].fd = %d, poll_map entry = %d i = %d max_poll_entry = %d\n",
        i, poll_array[i].fd, poll_map[poll_array[i].fd], i, max_poll_entry);
    }
  }
#endif

#ifdef LOCAL_DEBUG
  poll_array_verify(AFTER);
#endif /* LOCAL_DEBUG */
...
poll_array_verify(int before)
{
  ...
  if (print) {
    if (before) {
      printf("Array before compaction\n");
    } else {
      printf("Array after compaction\n");
    }
  }
  ...
  for (i = 0; i < max_poll_entry; i++) {
    ...
    if ((!before) && (fd == -1)) {
      printf("i = %4d fd = %6d map = %6d", i, fd, map);
      printf("  ERROR fd == -1 after compaction\n");
    }
...
epoll_event_del(int sd, enum which_set_enum which_set)
{
  ...
  PRINT_TIME(sd, &tnow, &tprev,
    "epoll_event_del: sd = %d op = 0x%x event = 0x%x", sd, epoll_op,
    ep_evt.events);
...
/* Done this way because nesting these macros doesn't work */
  ...
  TRACE(EVT_EPOLL_CTL,
    trace_fd = sd;
    rc = sys_epoll_ctl(epoll_fd, epoll_op, sd, &ep_evt); save_errno = errno;);
#else
  TRACE(EVT_EPOLL_CTL,
    trace_fd = sd;
    rc = epoll_ctl(epoll_fd, epoll_op, sd, &ep_evt); save_errno = errno;);
...
epoll_event_add(int sd, enum which_set_enum which_set)
{
  ...
  PRINT_TIME(sd, &tnow, &tprev,
    "epoll_event_add: sd = %d op = 0x%x event = 0x%x", sd, epoll_op,
    ep_evt.events);
...
find_epoll_array_entry(int sd)
{
  ...
  PRINT_TIME(sd, &tnow, &tprev, "find_epoll_array_entry: retval = %d",
    retval);
  return (retval);
}
...
free_epoll_array_entry(int sd)
{
  ...
  PRINT_TIME(sd, &tnow, &tprev,
    "free_epoll_array_entry: sd = %d op = 0x%x event = 0x%x", sd,
    EPOLL_CTL_DEL, ep_evt.events);
...
check_epollable(int sd)
{
  ...
  printf
    ("check_epollable epoll_ctl on sd = %d returns rc = %d and errno = %d\n",
    sd, rc, save_errno);
  if (rc < 0) {
    if (errno == EEXIST) {
      ...
      if (rc == 0) {
        ...
        printf
          ("check_epollable: epoll_wait reports sd %d has events = 0x%x\n",
          sd, ep_evt.events);
      }
    } else {
      printf
        ("check_epollable: epoll_ctl succeeded on sd = %d but got unexpected errno = %d\n",
        sd, save_errno);
    }
  } else {
    printf
      ("check_epollable FAILED CHECK on sd = %d -- not currently in interest set\n",
      sd);
  }
}
...
interest_set_print_info()
{
  ...
  switch (set_method) {
    ...
    default:
      printf
        ("interest_set_print_info: printing info for current method = %d not implemented yet\n",
        set_method);
      // exit(1);
      break;
  }
