...
do_ssl_accept(int sock)
{
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "do_ssl_accept: doing SSL_accept sock = %d\n", sock);
  if ((res = SSL_accept(ssl)) <= 0) {
    ...
    PRINT_TIME(NOFD, &tnow, &tprev,
      "do_ssl_accept: SSL_accept is successful ssl = %p sock = %d\n", ssl,
      sock);
...
int
is_ssl_listener(int listener_sd)
{
...
void
do_ssl_reneg(SSL * ssl)
{
  //do the renegotiation here
}

void
do_ssl_read(struct info *client_data, int amount_to_read)
{
  int num_bytes;
  int socket_flags;
  int error_no;
  num_bytes =
    SSL_read(client_data->ssl, client_data->req.buf, amount_to_read);
  error_no = errno;
...
void
do_ssl_write(struct info *client_data, int amount_to_write)
{
  long res = 0;
  int socket_flags;
  int my_error;
  ...
  while (client_data->rep.offset < client_data->rep.total_len) {
    ...
    res =
      SSL_write(client_data->ssl,
      &(client_data->rep.buf[client_data->rep.offset]),
...
/*---------------------------------------------------------------------*/
int
post_sock_ssl_accept(SSL * client_ssl, struct info *ip)
{
