...
stats_print_accept(void)
{
  ...
  printf("accept max reached       %12d   %5.1f %%\n",
    ...
    (double) num_accept_max_reached * 100.0 / (double) num_accept_successful);
  printf("accept ready but no idle %12d\n", num_accept_ready_but_no_idle);
  printf("\n");
  printf("accept num_conns and avg %12d   %5.1f (avg)\n",
    num_connections, (double) num_connections / (double) num_accept_calls);
...
stats_print_close(void)
{
  ...
  printf("Num sockets close calls  %12d   %5.1f %%\n",
    num_close_calls,
    (double) num_close_calls * 100.0 / (double) num_accept_successful);
  ...
  printf("Num sockets not closed   %12d   %5.1f %%\n",
    ...
    (double) (num_accept_successful - num_close_calls -
      num_close_failed) * 100.0 / (double) num_accept_successful);
  ...
  for (i = 0; i < FSM_STATE_MAX; i++) {
    if (num_closed_state[i] != 0) {
      printf("closed while in state %25s   %12d   %5.1f %%\n",
        ...
        (double) num_closed_state[i] * 100.0 / (double) num_close_calls);
    }
  }
  ...
  if (have_send_events()) {
    ...
    printf("Num close normal         %12d   %5.1f %%\n",
      ...
      (double) num_close_send_normal * 100.0 / (double) num_close_calls);
    printf("Num close early fin      %12d   %5.1f %%\n",
      num_close_send_early_fin,
      (double) num_close_send_early_fin * 100.0 / (double) num_close_calls);
    printf("Num close from err       %12d   %5.1f %%\n",
      num_pollerr_on_open_socket,
      (double) num_pollerr_on_open_socket * 100.0 / (double) num_close_calls);
    printf("\n");
  }
  ...
  printf("Num sockets close calls        %12d   %5.1f %%\n",
    ...
    (double) num_sock_close_calls * 100.0 / (double) num_accept_successful);
  printf("Num sockets close failed       %12d   %5.1f %%\n",
    num_sock_close_failed,
    (double) num_sock_close_failed * 100.0 / (double) num_sock_close_calls);
  printf("Num sockets close enotconn     %12d   %5.1f %%\n",
    num_sock_close_enotconn,
    (double) num_sock_close_enotconn * 100.0 / (double) num_sock_close_calls);
...
stats_print_read(void)
{
  ...
  printf("read EAGAIN              %12d   %5.1f %%\n",
    ...
    (double) num_socket_read_eagain * 100.0 / (double) num_socket_read_calls);
  ...
  printf("read RESET               %12d   %5.1f %%\n",
    ...
    (double) num_socket_read_reset * 100.0 / (double) num_socket_read_calls);
  ...
  printf("bytes per success read   %12.1f\n",
    (double) num_socket_read_bytes / (double) num_socket_read_successful);
...
stats_print_read_file(void)
{
  ...
  printf("bytes per success file read     %12.1f\n",
    (double) num_file_read_bytes / (double) num_file_read_successful);
  /* not quite the number of successful reads */
  printf("read successful file          %12d   %5.1f %%\n",
    num_file_read_successful,
    (double) num_file_read_successful * 100.0 / (double) num_file_read_calls);
  if (elapsed_time > 0) {
...
stats_print_write(void)
{
  ...
  if (num_socket_write_calls > 0) {
    ...
    printf("bytes per success write  %12.1f\n",
      (double) num_socket_write_bytes / (double) num_socket_write_successful);
...
stats_print_select(void)
{
  if (num_select_calls) {
    ...
    printf("select EINTR             %12d   %5.1f %%\n",
      ...
      (double) num_select_interrupted * 100.0 / (double) num_select_calls);
    printf("select EAGAIN            %12d   %5.1f %%\n",
      num_select_eagain,
      (double) num_select_eagain * 100.0 / (double) num_select_calls);
    printf("select other errors      %12d   %5.1f %%\n",
      num_select_eother,
      (double) num_select_eother * 100.0 / (double) num_select_calls);
    printf("select failed no errors  %12d   %5.1f %%\n",
      num_select_failed_noerror,
      (double) num_select_failed_noerror * 100.0 / (double) num_select_calls);
    printf("select timed out         %12d   %5.1f %%\n",
      num_select_timed_out,
      (double) num_select_timed_out * 100.0 / (double) num_select_calls);
    printf("select successful        %12d   %5.1f %%\n",
      num_select_successful,
      (double) num_select_successful * 100.0 / (double) num_select_calls);
    if (elapsed_time > 0) {
    ...
    printf("avg fds from select        %12.1f\n",
      (double) num_total_fds_from_select / (double) num_select_successful);
...
stats_print_poll(void)
{
  if (num_poll_calls) {
    ...
    printf("poll EINTR             %12d   %5.1f %%\n",
      ...
      (double) num_poll_interrupted * 100.0 / (double) num_poll_calls);
    printf("poll EAGAIN            %12d   %5.1f %%\n",
      num_poll_eagain,
      (double) num_poll_eagain * 100.0 / (double) num_poll_calls);
    printf("poll other errors      %12d   %5.1f %%\n",
      num_poll_eother,
      (double) num_poll_eother * 100.0 / (double) num_poll_calls);
    printf("poll failed no errors  %12d   %5.1f %%\n",
      num_poll_failed_noerror,
      (double) num_poll_failed_noerror * 100.0 / (double) num_poll_calls);
    printf("poll timed out         %12d   %5.1f %%\n",
      num_poll_timed_out,
      (double) num_poll_timed_out * 100.0 / (double) num_poll_calls);
    printf("poll successful        %12d   %5.1f %%\n",
      num_poll_successful,
      (double) num_poll_successful * 100.0 / (double) num_poll_calls);
    ...
    printf("avg fds from poll        %12.1f\n",
      (double) num_total_fds_from_poll / (double) num_poll_successful);
...
stats_print_epoll(void)
{
  if (num_epoll_calls) {
    ...
    printf("epoll EINTR             %12d   %5.1f %%\n",
      ...
      (double) num_epoll_interrupted * 100.0 / (double) num_epoll_calls);
    printf("epoll EAGAIN            %12d   %5.1f %%\n",
      num_epoll_eagain,
      (double) num_epoll_eagain * 100.0 / (double) num_epoll_calls);
    printf("epoll EPOLLHUP           %12d   %5.1f %%\n",
      num_epoll_epollhup,
      (double) num_epoll_epollhup * 100.0 / (double) num_epoll_calls);
    printf("epoll EPOLLERR           %12d   %5.1f %%\n",
      num_epoll_epollerr,
      (double) num_epoll_epollerr * 100.0 / (double) num_epoll_calls);
    printf("Note: EPOLLERR and EPOLLHUP often occur together\n");
    printf("epoll other errors      %12d   %5.1f %%\n",
      num_epoll_eother,
      (double) num_epoll_eother * 100.0 / (double) num_epoll_calls);
    printf("epoll failed no errors  %12d   %5.1f %%\n",
      num_epoll_failed_noerror,
      (double) num_epoll_failed_noerror * 100.0 / (double) num_epoll_calls);
    printf("epoll timed out         %12d   %5.1f %%\n",
      num_epoll_timed_out,
      (double) num_epoll_timed_out * 100.0 / (double) num_epoll_calls);
    printf("epoll successful        %12d   %5.1f %%\n",
      num_epoll_successful,
      (double) num_epoll_successful * 100.0 / (double) num_epoll_calls);
    ...
    printf("avg fds from epoll_wait      %12.1f\n",
      (double) num_total_fds_from_epoll / (double) num_epoll_successful);
    /* TODO 
    ...
    printf("avg fds to epoll_ctlv        %12.3f\n",
      (double) num_total_count_to_epoll_ctlv / (double) num_epoll_ctlv_calls);
...
stats_print_aio_wait(void)
{
  ...
  if (num_aio_wait_calls) {
    ...
    printf("aio_wait EAGAIN            %12d   %5.1f %%\n",
      num_aio_wait_eagain,
      (double) num_aio_wait_eagain * 100.0 / (double) num_aio_wait_calls);
    printf("aio_wait other errors      %12d   %5.1f %%\n",
      num_aio_wait_eother,
      (double) num_aio_wait_eother * 100.0 / (double) num_aio_wait_calls);
    ...
    printf("aio_wait timed out         %12d   %5.1f %%\n",
      ...
      (double) num_aio_wait_timed_out * 100.0 / (double) num_aio_wait_calls);
    printf("aio_wait successful        %12d   %5.1f %%\n",
      num_aio_wait_successful,
      (double) num_aio_wait_successful * 100.0 / (double) num_aio_wait_calls);
    if (elapsed_time > 0) {
...
stats_print_ifmodsince()
{
  ...
  percent =
    (double) num_reply_http_not_modified /
    (double) num_reqs_if_modified_since *100.0;
  printf("Number of HTTP 304 replies:%12d  %5.1f %%\n",
    num_reply_http_not_modified, percent);
  percent =
    (double) num_cached_fstat / (double) num_reqs_if_modified_since *100.0;
  printf("fstat() caching hit rate:                %5.1f %%\n", percent);
  percent =
    (double) num_extra_fstat / (double) num_reqs_if_modified_since *100.0;
...
stats_print_fcgi(void)
{
  ...
  if (num_fcgi_servers > 0) {
    ...
    printf(" avg bytes read                        %19.2f\n",
      (double) appserverstats.total_bytes_read /
      appserverstats.num_successful_reads);
    ...
    printf(" avg data bytes read                   %19.2f\n",
      (double) appserverstats.total_data_bytes_read /
      appserverstats.num_successful_data_reads);
    ...
    printf(" avg bytes written                     %19.2f\n",
      (double) appserverstats.total_bytes_written /
      appserverstats.num_successful_writes);
    ...
    if (num_fcgi_servers > 1) {
      for (i = 0; i < options.app_count; i++) {
        ...
        printf(" avg bytes read                        %19.2f\n",
          (double) app->serverstats.total_bytes_read /
          app->serverstats.num_successful_reads);
        appserver = app->server;
        if (appserver->next != NULL) {
          do {
            printf("  from %-32s%19.2f\n",
              appserver->path,
              (double) appserver->stats.total_bytes_read /
              appserver->stats.num_successful_reads);
            appserver = appserver->next;
          } while (appserver != NULL);
        }
        ...
        printf(" avg data bytes read                   %19.2f\n",
          (double) app->serverstats.total_data_bytes_read /
          app->serverstats.num_successful_data_reads);
        appserver = app->server;
        if (appserver->next != NULL) {
          do {
            printf("  from %-32s%19.2f\n",
              appserver->path,
              (double) appserver->stats.total_data_bytes_read /
              appserver->stats.num_successful_data_reads);
            appserver = appserver->next;
          } while (appserver != NULL);
        }
        ...
        printf(" avg bytes written                     %19.2f\n",
          (double) app->serverstats.total_bytes_written /
          app->serverstats.num_successful_writes);
        appserver = app->server;
        if (appserver->next != NULL) {
          do {
            printf("  to %-34s%19.2f\n",
              appserver->path,
              (double) appserver->stats.total_bytes_written /
              appserver->stats.num_successful_writes);
            appserver = appserver->next;
          } while (appserver != NULL);
        }
...
stats_print_encrypt()
{
  ...
  dv1 = (num_encrypt_tot_reqs == 0) ?
    0.0 : ((double) num_encrypt_encrypted / (double) num_encrypt_tot_reqs) *
    100.0;
  dv2 =
    (num_encrypt_size_ok ==
    0) ? 0.0 : ((double) num_encrypt_encrypted /
    (double) num_encrypt_size_ok) * 100.0;
