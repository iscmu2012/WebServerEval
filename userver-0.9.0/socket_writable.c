...
socket_writable(struct info *ip)
{
  ...
  if (send_reply(ip->sd, &ip->rep) < 0) {
    ...
    PRINT_TIME(ip->sd, &tnow, &tprev, "socket_writable: send_reply failed ");
    notification_on();
    ...
    case INFO_CLIENT:
      ...
      if (ip->skip == OPT_SKIP_DO_MADVISE) {
        if (ip->rep.nwritten > ip->skip_info + OPT_SKIP_MADVISE_SIZE) {
          ...
          PRINT_TIME(ip->sd, &tnow, &tprev,
            "post_socket_writable: madvise rc = %d nwritten = %zd "
            "total_len = %zd skip_info = %zd ", rc, ip->rep.nwritten,
            ip->rep.total_len, ip->skip_info);
          PRINT_TIME(ip->sd, &tnow, &tprev,
            "post_socket_writable: buf = %p start_addr = %p",
      ...
      if (ip->rep.nwritten >= ip->rep.total_len) {
        ...
        if (options.call_stats) {
          call_stats_finished(ip->req.num, ip->rep.total_len,
            ip->rep.nwritten, ip->req.client_id, ip->req.call_id,
            ip->rep.http_status);
        }
#endif /* CALL_STATS */
...
#ifdef UMEMREDUCTION
        // TODO:
        // Check with Tyler if other things should be done here, 
        // along the lines of what do_close is doing 
        ip->req.uri_len = 0;
#endif /* UMEMREDUCTION */
        ...
        if (ip->req.close) {
          PRINT_TIME(ip->sd, &tnow, &tprev,
            "post_socket_writable: closing %d", ip->sd);
          notification_on();
