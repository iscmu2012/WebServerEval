...
do_close(struct info *ip, int reason)
{
  ...
  if (ip->req.num == -1) {
    ...
    if (reason == REASON_CLIENT_CLOSED || reason == REASON_HTTP_1_0) {
      ...
      call_stats_failed(ip->req.num, ip->rep.total_len, ip->rep.nwritten,
        ip->req.client_id, ip->req.call_id, ip->rep.http_status, reason);
    }
  }
#endif
  ...
  if (get_fsm_state(sd) != FSM_FCGI_CLEARING_BUFFER) {
    interest_set_change(sd,
      ISET_NOT_READABLE | ISET_NOT_WRITABLE | ISET_DONE);
  } else {
  ...
  if (options.use_aio_close) {
    if (!options.aio_dont_close) {
      ...
      if (get_fsm_state(sd) == FSM_CONN_ERROR) {
        ...
        TRACE(EVT_AIO_SOCK_CLOSE,
          trace_fd = sd;
          PRINT_TIME(sd, &tnow, &tprev, "do_close: aio_sock_close sd = %d",
            sd); rc = aio_sock_close(sd););
        if (rc < 0) {
    ...
    if (ip->ssl) {
      ...
      PRINT_TIME(sd, &tnow, &tprev, "do_close: SSL_shutdown returns = %d",
        rc);
...
post_sock_close(int sd, int rc, int err)
{
  ...
  PRINT_TIME(sd, &tnow, &tprev,
    "post_sock_close: conn_num = %d requests = %d ", ip->conn_num,
    ip->req.num);
...
#ifdef UMEMREDUCTION
  //if tmp space has been allocated, free it here
  if (ip->req.tmp_allocated) {
    free(ip->req.tmp);
    ip->req.tmp_allocated = 0;
  }
  //if uri space has been allocated, free it here
  if (ip->req.uri_allocated) {
    free(ip->req.uri);
    ip->req.uri_allocated = 0;
  }
  //if cookie space has been allocated, free it here
  if (ip->req.cookie_allocated) {
    free(ip->req.cookie);
