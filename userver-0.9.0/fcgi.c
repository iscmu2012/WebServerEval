...
fcgi_find_idle(struct info *client_ip)
{
  ...
  PRINT_TIME(client_ip->sd, &tnow, &tprev, "queue_get returns %d",
    appserver_sd);
  if (appserver_sd >= 0) {
    ...
    PRINT_TIME(client_ip->sd, &tnow, &tprev,
      "found idle app server in idle_app_q sd = %d", appserver_ip->sd);
    return (appserver_ip);
  } else {
    ...
    do {
      ...
      if (appserver_ip->rep.fd == -1) {
        ...
        PRINT_TIME(client_ip->sd, &tnow, &tprev,
          "app server sd = %d is busy with client = %d", appserver_ip->sd,
          appserver_ip->rep.fd);
      }
      // num_busy++;
  ...
  if (appserver_ip == NULL) {
    ...
    PRINT_TIME(client_ip->sd, &tnow, &tprev,
      "returning appserver_ip->sd = %d", appserver_ip->sd);
  ...
  do {
    ...
    if (appserver_ip->rep.fd == -1) {
      PRINT_TIME(client_ip->sd, &tnow, &tprev, "found idle app server %d",
        appserver_ip->sd);
      mark_num_idle--;
      return appserver_ip;
    }
...
fcgi_find_idle(struct info *peer_ip)
{
  ...
  if (sd >= 0) {
    ...
    if (ip->rep.fd == -1) {
      ...
      mark_num_idle--;
      PRINT_TIME(sd, &tnow, &tprev, "found idle app server ip->sd = %d",
        ip->sd);
      return ip;
    }
  }
...
fcgi_service_request(struct info *ip, int peer_sd)
{
  ...
  PRINT_TIME(ip->sd, &tnow, &tprev, "fcgi_service_request: peer_sd = %d",
    peer_sd);
...
//printf("servicing using server %p\n", peer_ip);
  ...
  if (peer_ip->rep.dyn_buf == NULL) {
    ...
    PRINT_TIME(ip->sd, &tnow, &tprev,
      "fcgi_service_request: no dynamic buffers");
  ...
  if (!options.skip_header) {
    ...
    ldbg
      ("fcgi_service_request: fcgi_HTTP_hdr_len = %d peer_ip->rep.cur = %p\n",
...
fcgi_service_next_request(struct info *ip)
{
  ...
  PRINT_TIME(ip->sd, &tnow, &tprev,
    "fcgi_find_next_request returns next_sd = %d", next_sd);
  if (next_sd < 0) {
  ...
  PRINT_TIME(ip->sd, &tnow, &tprev, "queue_get returns next_sd = %d",
    next_sd);
  if (next_sd < 0) {
    PRINT_TIME(ip->sd, &tnow, &tprev, "no more requests");
    fcgi_go_idle(ip);
  ...
  PRINT_TIME(ip->sd, &tnow, &tprev,
    "try to service the next request next_sd = %d", next_sd);
...
fcgi_process_request(int client_sd)
{
  ...
  if (!queue_is_empty(app->req_q)) {
    PRINT_TIME(client_sd, &tnow, &tprev,
      "about to actually queue the request");
    stats->num_queued++;
  } else {
    PRINT_TIME(client_sd, &tnow, &tprev,
      "about to add request to queue for simplicity");
  }
  ...
  if (queue_put(app->req_q, client_sd) < 0) {
    ...
    if (appserver_ip != NULL) {
      PRINT_TIME(client_sd, &tnow, &tprev, "appserver_ip->sd = %d",
        appserver_ip->sd);
      ...
      if (next_sd < 0) {
        printf
          ("Tried to find the next request to process but failed even though one was just added\n");
        assert(0);
        exit(1);
      } else {
        PRINT_TIME(client_sd, &tnow, &tprev,
          "appserver_ip->sd = %d doing next_sd = %d", appserver_ip->sd,
          next_sd);
        retval = fcgi_service_request(appserver_ip, next_sd);
      }
    } else {
      PRINT_TIME(client_sd, &tnow, &tprev,
        "no idle appservers so delay handling request");
      retval = 0;
    }
  }
...
fcgi_add_to_reply_buffer_httpd(struct info *ip)
{
  ...
  do {
    ...
    switch (peer_ip->fsm_state) {
      case FSM_FCGI_WRITING_SET_VALUES:
        if (appserver->flags & APPSERVER_SHAREDMEM) {
          ...
          num_bytes = build_FCGI_SET_VALUES(rep->end,
            max_bytes,
            "SHAREDMEM_OFFSET", format_str("%d", sharedmem_offset), NULL);
      ...
      case FSM_FCGI_WRITING_BEGIN_REQUEST:
        num_bytes = build_FCGI_BEGIN_REQUEST(rep->end,
          max_bytes, requestId, FCGI_RESPONDER, FCGI_KEEP_CONN);
        if (num_bytes <= max_bytes) {
        ...
        switch (peer_req->method) {
          case HTTP_POST:
            num_bytes = build_FCGI_PARAMS(rep->end,
              max_bytes, requestId, "REQUEST_METHOD", "POST",
              //TODO: CONTENT_TYPE?
          ...
          case HTTP_HEAD:
            num_bytes = build_FCGI_PARAMS(rep->end,
              ...
              requestId,
              "REQUEST_METHOD", "GET",
              "QUERY_STRING", peer_req->query_string, NULL);
            break;
          default:
      ...
      case FSM_FCGI_WRITING_END_PARAMS:
        num_bytes = build_FCGI_PARAMS(rep->end, max_bytes, requestId, NULL);
        if (num_bytes <= max_bytes) {
        ...
        if (peer_ip->req.content_len > 0) {
          ...
          if (content_bytes > 0) {
            num_bytes = build_FCGI_STDIN(rep->end,
              max_bytes, requestId, content_bytes, peer_ip->req.cur);
            if (num_bytes > max_bytes) {
              /*
      ...
      case FSM_FCGI_WRITING_END_STDIN:
        num_bytes = build_FCGI_STDIN(rep->end, max_bytes, requestId, 0, NULL);
        if (num_bytes <= max_bytes) {
          interest_set_change(sd, ISET_NOT_WRITABLE);
    ...
    if ((int) num_bytes > rep->buf_size) {
      printf
        ("fcgi_add_to_reply_buffer_httpd(): we will never have enough room!\n");
      interest_set_change(sd, ISET_NOT_WRITABLE);
      ...
      ldbg
        ("fcgi_add_to_reply_buffer_httpd(): we currently don't have enough room\n");
      done = 1;
    } else {
...
fcgi_set_server_values(appserver_t * appserver)
{
  ...
  if (appserver->flags & APPSERVER_SHAREDMEM) {
    ...
    num_bytes = build_FCGI_SET_VALUES(buf, sizeof(buf),
      ...
      "SHAREDMEM_SIZE", format_str("%d", sharedmem_size), NULL);
    ...
    PRINT_TIME(NOFD, &tnow, &tprev,
      "Only fcgi_set_server_values only sends data if using shared memory");
  }
...
fcgi_find_next_request(struct info *client_ip, app_t * app)
{
  ...
  PRINT_TIME(client_ip->sd, &tnow, &tprev,
    "queue_get returns next_client_sd = %d", next_client_sd);
  if (next_client_sd > 0) {
    ...
    while (is_hungup(next_client_ip)) {
      PRINT_TIME(client_ip->sd, &tnow, &tprev,
        "client for next request timed out so skipping next_client_sd = %d",
        next_client_sd);
    ...
    PRINT_TIME(client_ip->sd, &tnow, &tprev,
      "nothing in the application request");
  }

  PRINT_TIME(client_ip->sd, &tnow, &tprev, "returning next_client_sd = %d",
    next_client_sd);
