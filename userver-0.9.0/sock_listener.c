  ...
  for (i = sock_listener_min; i <= sock_listener_max; i++) {
    if (sock_is_listener(i)) {
      ...
      if (ilp->maxconns <= 0) {
        ...
        printf
          ("We try to spread those connections evenly accross all listening sockets\n");
        printf("This includes sockets listening for SSL connections\n");
        printf
          ("The combination of the number of listening sockets you have created and"
...
sock_accept_more(int listen_sd)
{
  ...
  PRINT_TIME(listen_sd, &tnow, &tprev,
    "sock_accept_more: maxconns = %d", ilp->maxconns);
  PRINT_TIME(listen_sd, &tnow, &tprev,
    "sock_accept_more: max_thold = %d", ilp->max_thold);
  ...
  if ((ilp->aio_accept_outstanding + ilp->cur_conns) < ilp->maxconns) {
    PRINT_TIME(listen_sd, &tnow, &tprev,
