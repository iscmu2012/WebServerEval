...
int
aio_wait(int event_bitmap, compl_evt_t * evts, int max, struct timeval *tv)
{
  int evt_count = 0;
  static int state = 0;
    ...
    case 2:
      ...
      if (!options.use_sendfile && options.caching_on) {
        PRINT_TIME(NOFD, &tnow, &tprev,
          "aio_wait: trigger writev completion");
  ...
  PRINT_TIME(NOFD, &tnow, &tprev,
    "aio_wait: num_replies = %d MAX_REPLIES = %d", num_replies, MAX_REPLIES);
  return (evt_count);
} /* aio_wait */
