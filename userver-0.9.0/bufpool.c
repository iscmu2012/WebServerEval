...
alloc_bufpool(unsigned numblocks, size_t blocksize, int flags)
{
  ...
  if (bufpool != NULL) {
    ...
    if (bufpool->map != NULL) {
      printf
        ("alloc_bufpool: allocating numblocks = %u blocksize = %zu numbytes = %zu\n",
        numblocks, blocksize, numbytes);
      bufpool->buf =
        (char *) alloc_aligned_track(pagesize, numbytes, flags,
        "alloc_bufpool", &bufpool->track);
      if (bufpool->buf != NULL) {
        bufpool->flags = flags;
...
void
bufpool_free(bufpool_t * bufpool, void *buf)
{
  int index;
...
#ifdef QUEUE_BUFS
void *
check_finished_buf(struct dll_list *q, int sd)
{
  struct info *client;
  struct dll_element *el;
